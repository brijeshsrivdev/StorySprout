...
export default function Dashboard() {
  ...
  useEffect(() => {
    listProjects().then(async nextProjects => {
      setProjects(nextProjects);
      const entries = await Promise.all(nextProjects.map(async project => [project.id, await listStories(project.id).catch(() => [])] as const));
      setStories(Object.fromEntries(entries));
    }).catch(() => setError("We couldn't load your stories. Please try again.")).finally(() => setLoading(false));
  }, []);

  return <main className="min-h-screen bg-slate-50 text-slate-950"><div className="mx-auto max-w-6xl px-8 py-10">
    <header className="mb-10 flex items-center justify-between"><div><p className="text-sm font-semibold tracking-wide text-indigo-600">STORYSPROUT</p><h1 className="mt-2 text-4xl font-bold tracking-tight">Your stories</h1><p className="mt-2 text-slate-600">Create playful stories for little audiences.</p></div><Link href="/create" className="rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-slate-800">Create Story</Link></header>
