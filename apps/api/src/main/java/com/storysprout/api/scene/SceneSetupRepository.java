...
public class SceneSetupRepository {
    ...
    public SceneSetup insert(UUID id, UUID outlineSceneId) { var now=java.time.Instant.now(); jdbc.update("INSERT INTO scene_setups(id,outline_scene_id,created_at,updated_at) VALUES(?,?,?,?) ON CONFLICT (outline_scene_id) DO NOTHING",id,outlineSceneId,Timestamp.from(now),Timestamp.from(now)); return findByOutlineSceneId(outlineSceneId).orElseThrow(); }
