...
class StoryControllerIntegrationTest {
    ...
    @Test void databaseConstraintsProtectDomain() throws Exception { String projectId = createProject("Constraint Project"); org.assertj.core.api.Assertions.assertThatThrownBy(() -> jdbc.update("INSERT INTO stories (id, project_id, title, idea, target_age, duration_minutes, visual_style, language, creation_mode, generation_status, created_at, updated_at) VALUES (gen_random_uuid(), ?,'x','x','13_15',3,'2D','ENGLISH','BLANK','NOT_REQUESTED',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)", UUID.fromString(projectId))).isInstanceOf(org.springframework.dao.DataIntegrityViolationException.class); jdbc.update("INSERT INTO stories (id, project_id, title, idea, target_age, duration_minutes, visual_style, language, creation_mode, generation_status, created_at, updated_at) VALUES (gen_random_uuid(), ?,'valid','x','6_8',3,'2D','ENGLISH','BLANK','NOT_REQUESTED',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)", UUID.fromString(projectId)); org.assertj.core.api.Assertions.assertThatThrownBy(() -> jdbc.update("DELETE FROM projects WHERE id = ?", UUID.fromString(projectId))).isInstanceOf(org.springframework.dao.DataIntegrityViolationException.class); }
